...
def main():
    ...
    # 제목
    st.header("지하의 일상 기분 관리 비서")
    # 구분선
    st.markdown("---")

    # 기본 설명 (출판사 예제 그대로)
    with st.expander("일상 기분 관리 비서에 관하여", expanded=True):
        st.write(
            """   
               우울감이나 무기력을 느끼는 사용자의 일상과 업무를 돕는 음성 비서입니다.
